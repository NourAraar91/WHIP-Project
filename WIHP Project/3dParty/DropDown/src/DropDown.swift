...
public final class DropDown: UIView {
    ...
    public var isShown = PublishSubject<Bool>()
	//MARK: UI
	...
	*/
	@objc public dynamic var dropDownCornerRadius = DPDConstant.UI.CornerRadius {
		willSet {
    ...
    @objc public dynamic var borderColorX = DPDConstant.UI.SeparatorColor {
        willSet {
            tableViewContainer.borderColor = newValue
            tableView.borderColor = newValue
        }
        didSet { reloadAllComponents() }
    }
	...
	*/
	@objc public dynamic var dropDwonShadowOpacity = DPDConstant.UI.Shadow.Opacity {
...
private extension DropDown {
	...
	func setupUI() {
		...
		tableViewContainer.layer.cornerRadius = dropDownCornerRadius
		tableViewContainer.layer.shadowColor = dropDownShadowColor.cgColor
		tableViewContainer.layer.shadowOffset = shadowOffset
		tableViewContainer.layer.shadowOpacity = dropDwonShadowOpacity
		tableViewContainer.layer.shadowRadius = shadowRadius

		tableView.backgroundColor = tableViewBackgroundColor
		tableView.separatorColor = separatorColor
        tableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
		tableView.layer.cornerRadius = dropDownCornerRadius
		tableView.layer.masksToBounds = true
        tableView.borderWidth = borderWidth
        tableView.borderColor = borderColorX
        tableView.cornerRadius = dropDownCornerRadius
	}
...
extension DropDown {
	...
	fileprivate func computeLayout() -> (x: CGFloat, y: CGFloat, width: CGFloat, offscreenHeight: CGFloat, visibleHeight: CGFloat, canBeDisplayed: Bool, Direction: Direction) {
		...
		barButtonItemCondition: if let anchorView = anchorView as? UIBarButtonItem {
			...
			let width = self.widthX ?? fittingWidth()
	...
	fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = self.widthX ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - bottomOffset.x
	...
	fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = self.widthX ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - topOffset.x
